...
class JWT
{
    ...
    public static function sign(string $issuedBy, User $user)
    {
        ...
        return (new Builder())->issuedBy($issuedBy) // Configures the issuer (iss claim)
                              ...
                              ->withClaim(JWTConfig::JWT_USER_ID, $user->id) // Configures a new claim, called "uid"
                              ->withClaim(JWTConfig::JWT_USER_ID, $sanitizedUser)->withClaim(JWTConfig::ACL, serialize($acl))->withClaim(
            JWTConfig::JWT_REFRESH,
            $time->addSeconds(
                JWTConfig::JWT_REFRESH_CLAIM_EXPIRY_IN_SECONDS
            )->unix()
